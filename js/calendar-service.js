  ...
  function CalendarService($window, $http, $q) {
    ...
    var loadFile = function(urls) {
      ...
      angular.forEach(urls, function(url) {
        promises.push($http.get('/getcalendar'));
      });
...
/*
        if ((itm.end != undefined && (itm.end.isAfter(current_date) && itm.start.isBefore(current_date)))
              || itm.start.isBetween(current_date,end_date)){
                     future_events.push(itm);
        }
        오류로 인해서 수정하여 반영 - modify by Park Seok Ho
  */
        if (itm.start.isAfter(current_date) && itm.end.isBefore(end_date)){
                     future_events.push(itm);
        }
